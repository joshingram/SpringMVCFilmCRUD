		...
		<div class="container">
			<c:if test="${filmSearch}">
				<c:if test="${empty films}">
					<li class="error">No matching film found</li>
				</c:if>
			</c:if>
			<c:forEach var="film" items="${films}">
				...
				<ul>
					...
					<c:if test="${not empty film.plainLanguage}">
						<li>${film.plainLanguage}</li>
					</c:if>
				...
				<c:if test="${not empty film.id}">
					...
					<a href="viewInventory.do?filmId=${film.id}">View available
						inventory</a>
				</c:if>
			</c:forEach><br>
